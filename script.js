...
document.getElementById('confirm-ok').addEventListener('click', () => {
  ...
  confirmCallback?.(true); confirmCallback = null;
});
document.getElementById('confirm-cancel').addEventListener('click', () => {
  document.getElementById('confirm-modal').classList.add('hidden');
  confirmCallback?.(false); confirmCallback = null;
});
...
function initInstallPopup() {
  ...
  document.getElementById('learn-more-btn').onclick = () =>
    document.getElementById('install-features').scrollIntoView({ behavior: 'smooth' });
...
function initCookieBanner() {
  ...
  if (s) { applyConsent(s); return; }
  document.getElementById('cookie-banner').classList.remove('hidden');
  document.getElementById('accept-all-cookies').onclick = () => {
    saveCookieSettings({ analytics: true, preferences: true, thirdparty: true, marketing: true });
...
function setupAuthForms() {
  document.getElementById('to-register-btn').onclick = () => {
    ...
    document.getElementById('auth-subtitle').textContent = 'Konto erstellen';
  };
  document.getElementById('to-login-btn').onclick = () => {
    document.getElementById('form-register').classList.add('hidden');
    document.getElementById('form-login').classList.remove('hidden');
    document.getElementById('auth-subtitle').textContent = 'Willkommen zurück';
  };
  ...
  document.getElementById('reg-password').onkeydown = e => { if (e.key === 'Enter') doRegister(); };
  document.getElementById('back-to-login-btn').onclick = () => showScreen('auth-screen');
  document.getElementById('resend-btn').onclick = async () => {
    try {
      await authFetch('resend', { type: 'signup', email: verifyEmail });
      showStatus('verify-msg', 'success', '✓ Resend Link sent.');
    } catch (e) { showStatus('verify-msg', 'error', `✗ ${e.message}`); }
  };
}
...
async function doLogin() {
  ...
  if (!email || !password) { showStatus('login-msg', 'error', '✗ Bitte E-Mail und Passwort eingeben.'); return; }
  const btn = document.getElementById('login-btn');
  btn.disabled = true;
  showStatus('login-msg', 'info', '⏳ Anmeldung…', 0);
  try {
...
async function doRegister() {
  ...
  if (!name) { showStatus('register-msg', 'error', '✗ Bitte einen Anzeigenamen eingeben.'); return; }
  if (!email) { showStatus('register-msg', 'error', '✗ Bitte eine E-Mail-Adresse eingeben.'); return; }
  if (password.length < 8) { showStatus('register-msg', 'error', '✗ Das Passwort muss mindestens 8 Zeichen haben.'); return; }
  const btn = document.getElementById('register-btn');
  btn.disabled = true;
  showStatus('register-msg', 'info', '⏳ Konto wird erstellt…', 0);
  try {
    await authFetch('signup', { email, password, data: { display_name: name } });
    verifyEmail = email;
...
async function loadProfileAndEnter() {
  ...
  if (!userId) { doLogout(); return; }
  try {
    let rows = await sbFetch(`profiles?id=eq.${userId}&select=*`);
    if (!rows || rows.length === 0) {
      const displayName = currentSession.user?.user_metadata?.display_name || currentSession.user?.email?.split('@')[0] || 'Nutzer';
      rows = await sbFetch('profiles', {
        method: 'POST',
        headers: { Prefer: 'return=representation' },
        body: JSON.stringify({ id: userId, display_name: displayName, family_id: null })
      });
    }
...
async function loadFamily() {
  ...
  } catch (e) { console.error('loadFamily error:', e); }
}
...
function ensureCurrentUserInMemberMap() {
  ...
  if (!alreadyIn) {
    familyMembers.push({ id: currentSession.user.id, display_name: currentProfile?.display_name || 'Du' });
  }
}
  ...
  if (currentProfile.family_id && currentFamily) {
    ...
    document.getElementById('category-list').innerHTML = `
      <div class="empty-state" id="empty-state">
        ...
        <span>Füge eine Kategorie und Artikel hinzu!</span>
      </div>`;
  }
}

function updateAppHeader() {
  document.getElementById('family-name-display').textContent = currentFamily?.display_name || 'Keine Familie';
}

document.getElementById('logout-btn').onclick = doLogout;

function doLogout() {
  currentSession = null; currentProfile = null; currentFamily = null;
  familyMembers = []; categories = []; items = [];
...
function buildCategoryCard(cat, catItems, map) {
  ...
  card.innerHTML = `
    ...
    <div class="items-list" id="items-${cat.id}">
      ${catItems.length === 0
        ? '<div class="no-items-placeholder">Keine Items in dieser Kategorie</div>'
  ...
  card.querySelector('.delete-cat-btn').addEventListener('click', async e => {
    ...
    const ok = await confirmDialog('Kategorie löschen', `"${name}" und alle zugehörigen Items wirklich löschen?`);
  ...
  card.querySelectorAll('.item-delete-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      const ok = await confirmDialog('Artikel löschen', `"${btn.dataset.itemName}" wirklich löschen?`);
...
function buildItemHTML(item, map) {
  ...
  const adderName = item.added_by_user_id
    ? (isMe ? 'Du' : (map[item.added_by_user_id] || '?'))
    : '';
  ...
  return `
    <div class="item-row" id="item-${item.id}">
      <button class="item-check-btn ${doneClass}" data-item-id="${item.id}" data-done="${item.done}" title="${item.done ? 'Offen markieren' : 'Abhaken'}">
        <i class="fa-solid fa-check"></i>
      </button>
      <span class="item-label ${textClass}">${escHtml(item.name)}</span>
      ${qty}
      ${attribution}
      <button class="item-delete-btn" data-item-id="${item.id}" data-item-name="${escHtml(item.name)}" title="Löschen">
...
document.getElementById('category-input').onkeydown = e => { if (e.key === 'Enter') addCategory(); };
...
async function addCategory() {
  ...
  if (categories.some(c => c.name.toLowerCase() === name.toLowerCase())) {
    showStatus('category-msg', 'error', `✗ Kategorie "${name}" existiert bereits.`);
    return;
  }
  ...
  try {
    const rows = await sbFetch('categories', {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
      body: JSON.stringify({ name, family_id: currentProfile.family_id })
    });
    input.value = '';
    if (rows?.[0]) categories.push(rows[0]);
    renderAll();
    showStatus('category-msg', 'success', `✓ "${name}" hinzugefügt.`);
  } catch (e) { showStatus('category-msg', 'error', `✗ ${e.message}`); }
  finally { btn.disabled = false; }
}

document.getElementById('add-item-btn').onclick = addItem;
document.getElementById('item-name-input').onkeydown = e => { if (e.key === 'Enter') addItem(); };
...
async function addItem() {
  ...
  if (!name) { showStatus('item-msg', 'error', '✗ Bitte einen Artikelnamen eingeben.'); return; }
  if (!catId) { showStatus('item-msg', 'error', '✗ Bitte eine Kategorie auswählen.'); return; }
  ...
  try {
    const rows = await sbFetch('items', {
      ...
      headers: { Prefer: 'return=representation' },
      body: JSON.stringify({
        name, category_id: catId, family_id: currentProfile.family_id,
        quantity: qty, done: false,
    ...
    renderAll();
    showStatus('item-msg', 'success', `✓ "${name}" hinzugefügt.`);
  } catch (e) { showStatus('item-msg', 'error', `✗ ${e.message}`); }
  finally { btn.disabled = false; }
}
...
async function toggleItem(id, currentDone) {
  ...
  if (btn) {
    ...
    btn.title = newDone ? 'Offen markieren' : 'Abhaken';
  }
  ...
  try {
    ...
    if (btn) { btn.classList.toggle('done', currentDone); btn.dataset.done = String(currentDone); }
...
async function deleteCategory(catId) {
  try {
    await sbFetch(`items?category_id=eq.${catId}`, { method: 'DELETE' });
    await sbFetch(`categories?id=eq.${catId}`, { method: 'DELETE' });
...
function openProfileModal() {
  ...
  if (currentFamily) {
    ...
    familyMembers.forEach(m => {
      ...
      chip.innerHTML = `<i class="fa-solid fa-user"></i>${escHtml(m.display_name)}${isMe ? ' (Du)' : ''}`;
...
document.getElementById('save-display-name-btn').onclick = async () => {
  ...
  try {
    ...
    showStatus('name-msg', 'success', '✓ Name aktualisiert.');
  } catch (e) { showStatus('name-msg', 'error', `✗ ${e.message}`); }
};

document.getElementById('create-family-btn').onclick = async () => {
  const name = document.getElementById('new-family-name').value.trim();
  if (!name) { showStatus('family-msg', 'error', '✗ Bitte einen Familiennamen eingeben.'); return; }
  ...
  try {
    ...
    const fam = await sbFetch('families', {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
      body: JSON.stringify({ id: familyId, display_name: name })
    });
    currentFamily = fam[0];
    await sbFetch(`profiles?id=eq.${currentSession.user.id}`, {
      method: 'PATCH',
    ...
    familyMembers = [{ id: currentSession.user.id, display_name: currentProfile.display_name }];
    showStatus('family-msg', 'success', `✓ Familie "${name}" erstellt!`);
    document.getElementById('new-family-name').value = '';
    openProfileModal();
    enterApp();
  } catch (e) { showStatus('family-msg', 'error', `✗ ${e.message}`); }
  finally { btn.disabled = false; }
};

document.getElementById('join-family-btn').onclick = async () => {
  const id = document.getElementById('join-family-id').value.trim();
  if (id.length !== 64) { showStatus('family-msg', 'error', '✗ Die Familien-ID muss genau 64 Zeichen lang sein.'); return; }
  ...
  try {
    ...
    if (!fam || fam.length === 0) { showStatus('family-msg', 'error', '✗ Familie nicht gefunden.'); return; }
    await sbFetch(`profiles?id=eq.${currentSession.user.id}`, {
      method: 'PATCH',
      headers: { Prefer: 'return=representation' },
    ...
    currentFamily = fam[0];
    await loadFamily();
    showStatus('family-msg', 'success', `✓ Erfolgreich "${currentFamily.display_name}" beigetreten!`);
...
document.getElementById('leave-family-btn').onclick = async () => {
  const ok = await confirmDialog('Familie verlassen', `Willst du "${currentFamily?.display_name}" wirklich verlassen?`);
  if (!ok) return;
  try {
    await sbFetch(`profiles?id=eq.${currentSession.user.id}`, {
      method: 'PATCH',
...
document.getElementById('copy-id-btn').onclick = async () => {
  ...
  try {
    ...
    showStatus('family-msg', 'success', '✓ ID in die Zwischenablage kopiert!');
  } catch { showStatus('family-msg', 'info', 'ℹ Kopieren nicht unterstützt – bitte manuell kopieren.'); }
};

function startRealtime() {
  if (!currentProfile?.family_id) return;
  clearTimeout(reconnectTimer);
  if (realtimeSub) { realtimeSub.onclose = null; realtimeSub.onerror = null; realtimeSub.close(); realtimeSub = null; }
  try {
    ...
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({ topic: `realtime:public:items:family_id=eq.${fid}`, event: 'phx_join', payload: { config: { broadcast: { self: false }, presence: { key: '' } } }, ref: 'items' }));
      ws.send(JSON.stringify({ topic: `realtime:public:categories:family_id=eq.${fid}`, event: 'phx_join', payload: { config: { broadcast: { self: false }, presence: { key: '' } } }, ref: 'cats' }));
    };
    ws.onmessage = e => {
      try {
...
function setDot(color) {
  ...
  dot.parentElement.title = { green: 'Verbunden', red: 'Verbindung unterbrochen', gray: 'Nicht verbunden' }[color];
}

function startPolling() {
  stopPolling();
  pollTimer = setInterval(() => { if (currentProfile?.family_id) loadData(); }, POLL_INTERVAL);
}
...
async function handleHashToken() {
  ...
  try {
    ...
    currentSession = { access_token: accessToken, refresh_token: refreshToken, user };
    localStorage.setItem('shoply_session', JSON.stringify(currentSession));

    const splash = document.getElementById('splash');
    const brandEl = splash.querySelector('.splash-brand');
    brandEl.textContent = type === 'signup' ? '✓ E-Mail bestätigt!' : '✓ Authentifiziert!';
    ...
    showScreen('auth-screen');
    setupAuthForms();
    showStatus('login-msg', 'error', '✗ Bestätigungslink ungültig oder abgelaufen. Bitte erneut anmelden.');
    return true;
  }
}
